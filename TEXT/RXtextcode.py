#code yang sudah fix dari texttry
import numpy as np
import cv2
import time
import os
...
with open('Rxtext.txt', 'w') as f:
    while(True):
        ...
        # print(len(bit),'==>',bit)   ## ngeprint jumlah bit
            #print("Number of Contours found = " + str(number)) #ngeprint jumlah LED
            img1=cv2.resize(img,(img.shape[0]*4,img.shape[1]*4))
            #cv2.imshow('Pix',img1) 
            # my_track_method.init(frame, select_box) #ngezoom box yang ditrack
            
        # temp= untuk menampilkan huruf yang terbaca oleh camera (hanya menampilka perkarakter)
            tem = bit[8:16]
